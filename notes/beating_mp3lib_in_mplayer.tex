...
Native binaries on Duron and K6-3 ... and my Laptop Core2Duo at 800 MHz, 64 bit mode.
\begin{tabular}{lrrr}
build                     & Duron   & K6-III+ & Core 2 \\
\hline
variant1-dynamic (1.12.1) & 27.74 s &        &         \\
variant1-dynamic (1.13.4) &         &        & 16.72 s \\
variant1-dynamic (trunk)  & 27.81 s &        & 16.87 s \\
variant2-dynamic          & 27.10 s &        & 16.54 s \\
variant2-static           & 27.01 s &        & 16.51 s \\
mp3lib                    & 26.45 s &        & 18.56 s \\
ffmp3                     & 73.52 s &        & 22.00 s \\
ffmp3float                & 30.74 s &        & 12.26 s \\
variant2-float            &         &        & 18.76 s
\end{tabular}
What is the deal with ffmp3float on my Core2Duo? I have to check the output of that.
...
It is suspious that ffmp3float is not consistent. Repeated runs show differing numbers. Most interestingly such a run:
\begin{verbatim}
