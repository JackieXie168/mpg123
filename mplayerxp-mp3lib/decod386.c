 ...
 *
 * mp3_scaler - idea and implementation by Nick Kurshev <nickols_k@mail.ru>
 */
...
#define WRITE_SAMPLE_F(samples,sum) { *(samples) = sum*mp3_scaler; }
#endif

static int synth_1to1_mono_16(real *bandPtr,unsigned char *samples,int *pnt)
{
  ...
  ret = synth_1to1_16(bandPtr,0,(unsigned char *) samples_tmp,&pnt1);
  samples += *pnt;
...
static int synth_1to1_mono_32(real *bandPtr,unsigned char *samples,int *pnt)
{
...
static int synth_1to1_mono2stereo_16(real *bandPtr,unsigned char *samples,int *pnt)
{
  int i,ret;

  ret = synth_1to1_16(bandPtr,0,samples,pnt);
...
static int synth_1to1_mono2stereo_32(real *bandPtr,unsigned char *samples,int *pnt)
{
  int i,ret;
...
static int synth_1to1_l_32(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
  int i,ret;
...
static int synth_1to1_l_16(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
  int i,ret;

  ret = synth_1to1_16(bandPtr,channel,out,pnt);
  out = out + *pnt - 128;
...
static int synth_1to1_r_16(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
  int i,ret;
...
static int synth_1to1_r_32(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
  int i,ret;
...
int synth_1to1_MMX16( real *bandPtr,int channel,short * samples)
{
    static short buffs[2][2][0x110];
    static int bo = 1;
    synth_1to1_MMX_s_16(bandPtr, channel, samples, (short *) buffs, &bo); 
    return 0;
} 
#endif
static int synth_1to1_16(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
...
static int synth_1to1_32(real *bandPtr,int channel,unsigned char *out,int *pnt)
{
  static real buffs[2][2][0x110];
  static const int step = 2;
  static int bo = 1;
